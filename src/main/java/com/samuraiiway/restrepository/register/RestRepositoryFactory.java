...
public class RestRepositoryFactory {
    ...
    public static <T> T createProxy(Class<T> type, RestRepositoryAdvisor restRepositoryAdvisor) {
        ...
        proxyFactory.addAdvice(new RestRepositoryInvoker(restRepositoryAdvisor));

        return type.cast(proxyFactory.getProxy());
    }

    private static class RestRepositoryInvoker implements MethodInterceptor {

        private final RestRepositoryAdvisor restRepositoryAdvisor;
        private static final Pattern pattern = Pattern.compile("\\$\\{(\\w+)\\}");

        private RestRepositoryInvoker(RestRepositoryAdvisor restRepositoryAdvisor) {
